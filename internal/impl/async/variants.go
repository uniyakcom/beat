// SPSC Sharded Scheduler — 每个 P 一个 SPSC ring，worker 按亲和性消费
//
// 架构核心：
...
//
// SPSC 安全保证：
//   - 生产者: procPin 在 Enqueue 全程保持（pin → write → unpin）
//     同一 P 上只有一个 M 运行，procPin 禁止抢占 → 串行单写者
//   - 消费者: worker 静态分配 ring 集合，无 work-stealing → 单读者
//   - 两者并发安全：producer/consumer 在不同核心，通过 atomic Load/Store 通信
//
// Worker 分配策略：
